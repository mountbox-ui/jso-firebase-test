...
function jso_setup() {
	/*
		...
		* If you're building a theme based on JSO, use a find and replace
		* to change 'jso' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'jso', get_template_directory() . '/languages' );
...
add_action( 'after_setup_theme', 'jso_setup' );
 ...
 */
function jso_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'jso_content_width', 640 );
}
add_action( 'after_setup_theme', 'jso_content_width', 0 );
 ...
 */
function jso_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'jso' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'jso' ),
...
add_action( 'widgets_init', 'jso_widgets_init' );
...
add_action( 'wp_enqueue_scripts', 'jso_scripts' );
