<?php
//  METROPOLITIANS SHORTCODE
function jso_metropolitans_list_shortcode()
{

    $metropolitans = fetch_firebase_data('clergy/metropolitans');

    $placeholder = get_template_directory_uri() . '/assets/images/metropolitan.jpg';
    $uniq = uniqid('metro_');

    if (empty($metropolitans)) {
        return '<p class="text-center text-gray-500">No Metropolitans found.</p>';
    }
    ...
    <div class="mx-auto my-6 mt-5 mb-10">
        <div class="mb-6 sm:my-5 max-w-lg mx-auto">
            <input id="<?php echo $uniq; ?>_search" type="text" placeholder="Search Metropolitan..."
    ...
    <ul id="<?php echo $uniq; ?>_list" role="list" class="divide-y divide-gray-100 transition-all">
        ...
        <?php foreach ($metropolitans as $id => $m):
            $name = $m['fatherName'] ?? 'Unknown Metropolitan';
            $role = $m['vicarAt'] ?? 'Metropolitan';
            ...
            <li class="metro-item flex justify-between gap-x-6 py-5 cursor-pointer hover:bg-gray-50 transition"
                data-name="<?php echo strtolower(esc_attr($name)); ?>" data-role="<?php echo strtolower(esc_attr($role)); ?>"
                data-modal-name="<?php echo esc_attr($name); ?>" data-modal-role="<?php echo esc_attr($role); ?>"
                data-image="<?php echo esc_attr($image); ?>" data-email="<?php echo esc_attr($email); ?>"
                data-phone="<?php echo esc_attr($phone); ?>" data-diocese="<?php echo esc_attr($diocese); ?>">
    ...
    <p id="<?php echo $uniq; ?>_no_results" class="hidden text-center text-gray-500 mt-4">
        No matching metropolitans found.
    </p>
